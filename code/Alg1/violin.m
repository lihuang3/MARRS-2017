...
%}
%%
%function[h,p,L,MX,MED,bw]=violin(Y,varargin)
function[h,p]=violin(Y,varargin)  % LH
%defaults:
...
plotmedian=1;
x = [];
if isempty(find(strcmp(varargin,'xoffset')))==0
    xoffset = varargin{find(strcmp(varargin,'xoffset'))+1};
end

if isempty(find(strcmp(varargin,'xgap')))==0
    xgap = varargin{find(strcmp(varargin,'xgap'))+1};
end
x=xgap.*linspace(1+xoffset,10+xoffset,10); % LH
...
for i=i:size(Y,2)
    ...
    elseif setX == 1
        if plotmean == 1
            p(1)=plot([interp1(U(:,i),F(:,i)+i,MX(:,i))+x(i)-i, interp1(flipud(U(:,i)),flipud(i-F(:,i)),MX(:,i))+x(i)-i],[MX(:,i) MX(:,i)],mc,'LineWidth',2,'MarkerSize',2);
        end
        if plotmedian == 1
...
elseif setX == 1
    axis([min(x)-0.1*range(x) max(x)+0.1*range(x), min(U(:)) max(U(:))]);
