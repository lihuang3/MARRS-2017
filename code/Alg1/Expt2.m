...
h = scatter(ptc(:,1),ptc(:,2),35,'filled');
h.CData = [0,0.3,0.3];
...
while(pbinCost>10*SwarmSize)%(new_p.neighbor>0)
    ...
    if (NextParticle >= 1)  % Select new object to move
        ...
        % Update the region we are looking at
        ...
        for ii = 1:size(LocalSwm,1)
           ...
           % branch member +1
            ...
            if tree(parent,7) >0 && tree(parent,7)~=Ijt(end) && (Ijt_cnt<numel(Ijt)-RegIntRange)    
...
% disp('Collection Percentage:')
 %disp(100*numel(LocalSwm)/SwarmSize)
