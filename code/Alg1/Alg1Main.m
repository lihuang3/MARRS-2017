...
while(pbinCost>10*SwarmSize)%(new_p.neighbor>0)
    ...
    if (NextParticle == 1)  % Select new object to move
        ...
        if(~ (Ijt_cnt>0 && numel(find(Pdist == Ijt(Ijt_cnt)))./SwarmSize > 0))
        ...
        % more than 5% of the population then do aggregation
            while (Ijt_cnt<=10)
                Ijt_cnt = Ijt_cnt+1;
                if numel(find(Pdist == Ijt(Ijt_cnt)))./SwarmSize > 0
                    break;
                end
       ...
       %  []~(???)~*
        ...
        if Np == 0 % No particles in the priority zone
            ...
            if(Ijt_cnt<numel(Ijt)) 
                ...
                [ActVal1 ActID1] = min(JTbranch_temp(:,3));
                ActID2 = find(SwarmProp(:,3) ==ActID1);
                [ActVal3 ActID3] = min(SwarmProp(ActID2,4));
            end
...
%             h.YData(Np) = dist(2);
    ...
    %             scatter(n,entro,12,'filled');
            if(Ijt_cnt < numel(Ijt))
                ...
                pbinX =  1+floor(h.XData(LocalSwm)'/bin.unitX); 
                pbinY =  1+floor(h.YData(LocalSwm)'/bin.unitY);
            end
...
% disp(Expt)
 ...
 disp(n)
 disp('Collection Percentage:')
 disp(numel(LocalSwm)/SwarmSize)
