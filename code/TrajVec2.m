...
Init = 1;
figure(3); hold on
temp1 = ConnMat(1,2*1:2*1+1);
h = scatter(temp1(2),temp1(1),10,'filled')
h.CData = [0 1 0];
            ...
            if cost1 > 100
                if flag==0
                    while(cost_diff == 0 || cost_diff<MaxStep && Brch0(temp2(1),temp2(2))==0)
                        ...
                        nb0 = Brch0(-1+temp2(1):1+temp2(1),-1+temp2(2):1+temp2(2));
                        nb0(2,2)=0;
                        
                        if ~isempty(find(nb0==1)) 
                            [row col] = find(nb0==1);
                            row = row-2;
                            col = col-2;
                            temp = [row(1)+temp2(1),col(1)+temp2(2)];
                            if Pathway(temp(1),temp(2)) > cost
